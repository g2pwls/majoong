...
// ------------------------------------------------------------------
// /support (목장 후원) 페이지 — 사용자의 Navbar/Breadcrumb 컴포넌트 사용
// ------------------------------------------------------------------
...
const TempBadge: React.FC<{ temp: number }> = ({ temp }) => {
  const color = temp >= 60 ? "bg-green-500" : temp >= 45 ? "bg-blue-400" : temp >= 38 ? "bg-yellow-400" : "bg-red-500";
  return (
    <div className={`ml-6 absolute -left- rounded-full px-2 py-1 text-xs font-semibold text-white ${color}`}>
      {temp.toFixed(1)}°C
    </div>
  );
};
...
export default function SupportPage() {
  ...
  // 단일 농장 API를 조회하고, 해당 농장의 말 이미지 4장을 채워넣음
  React.useEffect(() => {
    let alive = true;
    (async () => {
      try {
        setLoading(true);
        const resFarm = await fetch(`/api/farms/demo`, { cache: "no-store" });
        const farm = (await resFarm.json()) as any;

        // horse 이미지 4장 조회
        const farmId = farm?.farm_uuid ?? farm?.id ?? "demo";
        const resHorses = await fetch(`/api/horse/${farmId}`, { cache: "no-store" });
        const horses = (await resHorses.json()) as any[];
        const horseImages = horses.slice(0, 4).map((h) => h.horse_url).filter(Boolean);
        ...
        const mapped: Farm = {
          ...
          horse_url: horseImages,
        };
        if (alive) setFarms([mapped]);
      } catch (e) {
        console.error(e);
        if (alive) setFarms([]);
      } finally {
